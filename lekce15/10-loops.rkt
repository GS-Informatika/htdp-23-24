...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname 10-loops) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
; ------- CYKLY -------
...
;;    naše abstrsakce jsou založeny na tvorbě nových dat
;;    z procházení "již existujících dat"



;; Vysvětlete - jaká jsou existující data v případě funkce
#; (build-list 20
               (lambda (x) x))
...
;; for loop konstrukce, které akumulují (collect, aggregate) do jiných datových
;; typů
...
;; Cykly přes čísla nejsou vždy jen enumerace 0 ... n-1,
;; programy často potřebují procházet sekvence čísel např.
;; s pevým krokem, nebo dopředu nevíme, kolik čísel musíme
;; projít. K tomu slouží funkce které generují sekvence
#; (for/list ([i (in-range 6 20 2)])
     i)

#; (for/list ([lst '(a b c d e f g h)]
              [n (in-naturals 3)])
     n)